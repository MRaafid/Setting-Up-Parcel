...
addTaskInput.addEventListener("keyup", (evt)=>{
    ...
    if (evt.key === "Enter") {
        ...
        if (newTask) {
            (0, _model.addTask)(newTask);
            evt.target.value = "";
            (0, _viewDefault.default)((0, _model.getTasks)(), taskList);
        }
    }
});
...
const itemTemplate = (id, task)=>`<div class="task">${task}<div class="rem-btn" data-id="${id}">\u{274C}</div></div>`;
function render(tasks, taskList) {
    const content = tasks.map((t)=>itemTemplate(t.id, t.task));
    taskList.innerHTML = content.join("");
}

},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["4rkIz","8lqZg"], "8lqZg", "parcelRequire2d1f")
